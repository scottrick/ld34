...
BoundarySystem.prototype.handleEntity = function(scene, entity, deltaTime) {
	...
	if (transform.position.x + transform.scale.x * 0.5 < this.boundary.x) {
		scene.removeEntity(entity);
		return;
	}

	if (transform.position.x - transform.scale.x * 0.5 > this.boundary.x + this.boundary.w) {
		scene.removeEntity(entity);
		return;
	}

	if (transform.position.y + transform.scale.y * 0.5 < this.boundary.y) {
		scene.removeEntity(entity);
		return;
	}

	if (transform.position.y - transform.scale.y * 0.5 > this.boundary.y + this.boundary.h) {
		scene.removeEntity(entity);
		return;
	}
}
